...
4. 2曲目以降は前の曲のテンポに近づくようタイムストレッチ(伸縮率の上限は内部で自動設定、極端な音程/速度変化を避ける)
5. 前の曲の終盤と次の曲の頭を、検出したビート位置に合わせて重ね、イコールパワー(等音量感)カーブでクロスフェード
6. `--workout` / Web版のチェックボックスを付けた場合、ベースを持ち上げてコンプレッサーをかけ、ビートを強めに聴かせるマスタリングを追加
7. 最終的なミックスを指定した形式(mp3/wavなど)で書き出し
...
| `--speeds` | 全曲1.0倍 | 曲ごとの再生速度倍率をカンマ区切りで指定(ピッチは保持)。`inputs`と同じ順番。指定しなかった曲は1.0倍。範囲は0より大きく1.5以下 |
